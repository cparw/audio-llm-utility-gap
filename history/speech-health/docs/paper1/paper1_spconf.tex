...
\begin{figure}[!t]
\centering
\includegraphics[width=0.88\columnwidth]{plot1_encoder_layers.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.7\columnwidth]{plot_projector_two_datasets.png}
...
from 0.70 to 0.89 on the agreement segments. On the conflict segments the readout reaches 0.61 (a
...
partial and does not follow language: the two Spanish sets transfer to each other (0.46, 0.66) worse
than to English MDVR-KCL (0.86, 0.74).}
\label{tab:cross}
\begin{tabular}{@{}lccc@{}}
\toprule
...
interviewer speech and are re-cut on the cluster before camera-ready. The projector was retrained with encoder layer 24 as its input, so training and
layer choice are not yet separated; a control that retrains it on its default input layer is running,
and the readout result, which changes no layer, carries the claim on its own. On DAIC, scores track PTSD severity
as closely as depression severity (0.60 against 0.59): the models detect distress, not depression
specifically. Pitt and DAIC are distributed under their own agreements; we release
segment identifiers, scripts and the score of every clip, not audio or text.
