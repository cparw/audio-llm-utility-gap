#!/usr/bin/env python3
"""Text only on the Alzheimer's clips: qwen2 audio gets just the transcript, no audio."""
import csv, os, time
...
out = open(f"{B}/ad_textonly_scores.csv", "w", newline="")
...
for i, r in enumerate(rows):
    ...
    try:
        ...
        w.writerow([r["segment_path"], r["set"], r["label"], r["grp"]+r["spk"], py/t if t>0 else 0.5, t]); out.flush()
    except Exception as e:
        print(f"fail {r['segment_path']}: {repr(e)[:100]}", flush=True)
    if (i+1)%100==0: print(f"{i+1}/{len(rows)} {(time.time()-t0)/60:.1f} min", flush=True)
out.close()
rr = list(csv.DictReader(open(f"{B}/ad_textonly_scores.csv")))
def auc(y,p):
...
print(f"AD TEXTONLY overall AUC {auc(y,p):.3f} (audio answer 0.618, text scorer baseline 0.699)", flush=True)
for s in ("agreement","conflict"):
    m = [x["set"]==s for x in rr]
