...
for fold, (tr, te) in enumerate(gkf.split(np.zeros(len(rows)), y, groups=spk)):
    ...
    with torch.no_grad():
        for i in te:
            ...
            oof[i] = float(two[0])
    np.save(f"{D}/projector_retrain_oof_partial.npy", oof)
    print(f"fold {fold} eval done", flush=True)

np.save(f"{D}/projector_retrain_oof.npy", oof)
def rank_auc(yy, pp):
...
print(f"RETRAINED PROJECTOR overall AUC {rank_auc(y, oof):.3f}  (baseline answer 0.618, untrained plug 0.488)", flush=True)
