...
def score(proc,model,yes,no,audio,prompt):
    conv=[{"role":"user","content":[{"type":"audio","audio_url":"x"},{"type":"text","text":prompt}]}]
...
def main():
    ...
    # precompute a shuffled audio order (each clip gets a different speaker's audio, same text)
    ...
    for i,r in enumerate(rows):
        try:
            ...
            # shuffled control: SICK text but a DIFFERENT speaker's audio
    ...
    # shuffled control: does SICK score track text (high everywhere) regardless of audio?
    shuf_auc=sa("rel_sick_shuffled")
    summ=dict(tag=a.tag,n=len(out),cond=a.cond,lang=a.lang,tasks=a.tasks,speakers=len(set(o["speaker"] for o in out)),
