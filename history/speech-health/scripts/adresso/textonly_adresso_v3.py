...
rows = list(csv.DictReader(open(f"{D}/adresso/adresso_transcripts_v3.csv")))
out = open(f"{D}/adresso/adresso_textonly_v3_scores.csv", "w", newline="")
...
out.close()
rr = list(csv.DictReader(open(f"{D}/adresso/adresso_textonly_v3_scores.csv")))
def auc(y,p):
