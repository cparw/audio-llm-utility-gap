...
# Writes: one folder per run under part24/part12/final_runs/ on the external drive (log, flagged csv, page5.txt,
...
OUTD="checks/final_2325Z/part12/final_runs/${TAG}_${STAMP}"   # only change vs part24 final_check.sh: run folder under checks/final_2325Z/part12
if ! mkdir -p "$OUTD" 2>/dev/null; then
  OUTD="${TMPDIR:-/tmp}/part24_final_${TAG}_${STAMP}"; mkdir -p "$OUTD"
  echo "WARNING: external drive folder not writable; writing this run to $OUTD"
fi
