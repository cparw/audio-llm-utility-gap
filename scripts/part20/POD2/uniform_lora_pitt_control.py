"""PART20 POD2 CONTROL (uniform weights, w=1): identical to balanced_lora_pitt.py except the weight line. Same-pod, same-seed control for the BALANCED LoRA on Pitt 468 for Qwen2.5-Omni.
...
for k in MYFOLDS:
    ...
    w = np.where(np.isin(np.arange(len(rows)), tr), 1.0, 0.0); table = [dict(t, w_rescaled=1.0, total_weight=float(t['N_ya'])) for t in table]; scale = 1.0  # CONTROL: uniform
