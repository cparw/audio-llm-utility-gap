...
Load path identical to the 20 Sep pod af2_run.py: librosa.load(sr=16000, mono=True), no cut.
...
if not done:
    w.writerow(["clip_id", "orig_clip_id", "speaker_id", "label", "arm", "clip_path", "dur_s",
                "p_yes", "answer_mass", "p_yes_rule3", "answer_mass_rule3", "prompt"]); fh.flush()
t1 = time.time(); n = 0
print(f"{len(rows)} rows, {len(done)} already done", flush=True)
for i, r in enumerate(rows):
    ...
    w.writerow([r["clip_id"], r.get("orig_clip_id", r["clip_id"]), r["speaker"], r["label"], r["arm"], r["path"],
                round(len(x) / 16000, 3), py, den, y3 / (y3 + n3 + 1e-12), y3 + n3, P]); fh.flush(); n += 1
