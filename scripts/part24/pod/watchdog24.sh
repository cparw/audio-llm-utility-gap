#!/bin/bash
# PART24 watchdog. Runs on the Mac, independent of any other job. Every 3 min, for each pod in pods24.txt
...
R="<local data dir>/part24"
PODS="$R/pods24.txt"; PULL="$R/pull"; WD="$R/wd"; mkdir -p "$PULL" "$WD"
KEY=$(tr -d '\n\r ' < ~/.runpod_key)
