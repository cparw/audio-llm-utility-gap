#!/bin/bash
# PART25 A retry watchdog (adapted from part24/watchdog24.sh). Runs on the Mac, independent of any session. Every 3 min, for each pod in pods_r2.txt
...
# Exits when pods_r2.txt has no live pods.   usage: bash watchdog24.sh [once]
R="<local data dir>/release_from_mac/scores/part25/A/retry_tf554"
PODS="$R/pods_r2.txt"; PULL="$R/pull"; WD="$R/wd"; mkdir -p "$PULL" "$WD"
KEY=$(tr -d '\n\r ' < ~/.runpod_key)
...
ONCE=${1:-}
IDLE_S=900; STALL_S=3600; UNREACH_S=1800; MAXAGE_S=14400; BAL_MIN=4
...
pull_pod(){ # name ip port -> 0 when the tar holds exactly the pod's file list and the result csv hashes match
  ...
  cnt=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace && { find out logs -type f 2>/dev/null; ls -1 *.sh *.py out_sha256.txt data/mf_*.csv data/files.sha256 2>/dev/null;
      for m in PIP_DONE HF_DONE DATA_OK ALL_DONE FAILED; do [ -f $m ] && echo $m; done; } > /root/pull.list;
...
while true; do
  ...
  while read -r name id ip port cost <&3; do
    ...
    out=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace 2>/dev/null; a=0; [ -f ALL_DONE ] && a=1;
      ...
      f=-; [ -f FAILED ] && f=$(cat FAILED | tr " " "_"); dk=0; [ -f DATA_OK ] && dk=1;
      mk=$(ls PIP_DONE HF_DONE DATA_OK ALL_DONE 2>/dev/null | tr "\n" ",");
      echo "ALL=$a DATA=$dk GPU=${g:-NA} PY=$py LOGAGE=$((nw-${lm:-$nw})) FAILED=$f MK=$mk | $(tail -1 logs/DRIVER.log 2>/dev/null | cut -c1-150)"' 2>/dev/null | tr '\n' ' ')
    if [ -z "$out" ]; then
    ...
    DK=$(echo "$out" | sed -n 's/.*DATA=\([01]\).*/\1/p')
    if [ "$GPU" = "0" ] && [ "$PY" = "0" ] && [ "$DK" = "1" ]; then
  ...
  log "live pods: $live"
  if [ $live -le 0 ]; then log "no live pods in pods_r2.txt, watchdog exit"; exit 0; fi
