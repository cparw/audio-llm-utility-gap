#!/bin/bash
# PART25 A retry pod-side guard (independent of the Mac): REST-free podStop via the pod's own key keeps /workspace but stops
# GPU billing when ALL_DONE is older than 45 min (Mac watchdog should have pulled and deleted by then), or GPU 0% with
# no python for 40 min after setup, or after 5 h uptime.
eval $(cat /proc/1/environ | tr "\0" "\n" | grep -E "^RUNPOD_(POD_ID|API_KEY)=" | sed "s/^/export /")
L=/workspace/logs/guard_r2.log; idle=0; t0=$(date +%s)
...
while true; do
  ...
  if [ "$g" = "0" ] && [ "$py" = "0" ] && [ -f /workspace/HF_DONE ]; then idle=$((idle+60)); else idle=0; fi
