#!/bin/bash
# p16twins watchdog (adapted from part24/watchdog24.sh: pods.txt, pull/, CPU pods: no GPU so GPU=NA counts as idle; PL_DONE counts as ALL_DONE). Runs on the Mac, independent of any other job. Every 3 min, for each pod in pods.txt
...
R="<local data dir>/leftovers_23sep/p16twins"
...
while true; do
  ...
  while read -r name id ip port cost <&3; do
    ...
    live=$((live+1))
    out=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace 2>/dev/null; a=0; [ -f ALL_DONE ] && a=1; [ -f PL_DONE ] && a=1;
