...
DS, ST = sys.argv[1], sys.argv[2]; TAG = sys.argv[3] if len(sys.argv) > 3 else ""
R = 5
T0 = time.time()
...
with open(f"/workspace/out/twin_{DS}_{ST}{TAG}_perlayer.csv", "w", newline="") as fh:
...
np.savez_compressed(f"/workspace/out/twin_{DS}_{ST}{TAG}_best_oof.npz", oof=OOF[best], label=y, spk=spk, name=name, stage=best)
...
json.dump(out, open(f"/workspace/out/twin_{DS}_{ST}{TAG}.json", "w"), indent=1)
