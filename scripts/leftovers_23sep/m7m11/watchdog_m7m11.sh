#!/bin/bash
# m7m11 watchdog (adapted from part24/watchdog24.sh for CPU pods). Runs on the Mac, independent of any other job. Every 3 min, for each pod in pods24.txt
...
# Exits when pods.txt has no live pods.   usage: bash watchdog_m7m11.sh [once]
R="<local data dir>/leftovers_23sep/m7m11"
