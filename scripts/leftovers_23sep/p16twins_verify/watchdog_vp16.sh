#!/bin/bash
# p16twins VERIFIER watchdog (adapted from part24/watchdog24.sh for CPU pods: no GPU, so "GPU" is read as 0 and idle = no python and no file written under /workspace for 15 min). Runs on the Mac, independent of any other job. Every 3 min, for each pod in pods24.txt
...
R="<local data dir>/leftovers_23sep/verify/p16twins_work"
...
pull_pod(){ # name ip port -> 0 when the tar holds exactly the pod's file list and the result csv hashes match
  ...
  cnt=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace && { find out logs -type f 2>/dev/null; ls -1 *.sh *.py 2>/dev/null;
      for m in PIP_DONE DIR_DONE STAGE_DONE ALL_DONE FAILED; do [ -f $m ] && echo $m; done;
  ...
  ssh -n $SSHO -p $port root@$ip 'cd /workspace && sha256sum out/FT_*_oof.csv out/FT_*.sidecar.json 2>/dev/null' > "$WD/$n.remote.sha" 2>/dev/null
...
while true; do
  ...
  while read -r name id ip port cost <&3; do
    ...
    out=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace 2>/dev/null; a=0; [ -f ALL_DONE ] && a=1;
      ...
      lm=$(find /workspace -type f -printf "%T@\n" 2>/dev/null | sort -n | tail -1 | cut -d. -f1); nw=$(date +%s);
      f=-; [ -f FAILED ] && f=$(cat FAILED);
      mk=$(ls PIP_DONE DIR_DONE STAGE_DONE ALL_DONE 2>/dev/null | tr "\n" ",");
    ...
    if [ "$GPU" = "0" ] && [ "$PY" = "0" ] && [ -n "$LA" ] && [ "$LA" -ge 60 ] 2>/dev/null; then
