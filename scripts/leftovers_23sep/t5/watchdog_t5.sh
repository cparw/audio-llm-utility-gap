#!/bin/bash
# T5 (leftovers_23sep) watchdog, adapted from part24/watchdog24.sh for CPU pods. Every 3 min, for each pod in pods.txt
...
# Exits when pods.txt has no live pods.   usage: bash watchdog_t5.sh [once]
R="<local data dir>/leftovers_23sep/t5"
...
pull_pod(){ # name ip port -> 0 when the tar holds exactly the pod's file list and the result csv hashes match
  ...
  cnt=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace && { find out logs -type f 2>/dev/null; ls -1 *.sh *.py 2>/dev/null;
      for m in PIP_DONE ALL_DONE FAILED POD_NAME JOBS MODE VARIANTS; do [ -f $m ] && echo $m; done; } > /root/pull.list;
