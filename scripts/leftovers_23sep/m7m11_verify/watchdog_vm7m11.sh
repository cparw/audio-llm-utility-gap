#!/bin/bash
# verify m7m11 watchdog (adapted from part24/watchdog24.sh for CPU pods: GPU NA counts as idle, pods.txt, sha check on out/*.csv out/*.json). Runs on the Mac, independent of any other job. Every 3 min, for each pod in pods24.txt
...
R="<local data dir>/leftovers_23sep/verify/m7m11_work"
