"""Independent verifier refit (own code, t5 verify). Per layer: StandardScaler fit on the training fold,
...
if __name__ == "__main__":
    ...
    for key in STREAMS.split(","):
        ...
        with Pool(NP) as pool: rows = sorted(pool.map(fit_layer, range(X.shape[1])), key=lambda r: r[0])
