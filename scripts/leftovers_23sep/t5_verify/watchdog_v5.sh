#!/bin/bash
# t5 VERIFIER (leftovers_23sep/verify/t5v) watchdog, adapted from part24/watchdog24.sh (via the t5 CPU-pod edits). Every 3 min, for each pod in pods.txt
...
#   no python (CPU pod, no GPU) >= 15 min -> pull the same way, delete, log "IDLE KILL".
#   no log write >= 45 min (python present, hung) -> pull, delete, log "STALL KILL".
...
# Exits when pods.txt has no live pods.   usage: bash watchdog_v5.sh [once]
R="<local data dir>/leftovers_23sep/verify/t5v"
...
ONCE=${1:-}
IDLE_S=900; STALL_S=2700; UNREACH_S=1800; MAXAGE_S=14400; BAL_MIN=4
...
pull_pod(){ # name ip port -> 0 when the tar holds exactly the pod's file list and the result csv hashes match
  ...
  cnt=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace && { find out logs -type f 2>/dev/null; ls -1 *.sh *.py 2>/dev/null;
      for m in PIP_DONE UPLOAD_DONE ALL_DONE FAILED POD_NAME JOBS; do [ -f $m ] && echo $m; done; } > /root/pull.list;
...
while true; do
  ...
  while read -r name id ip port cost <&3; do
    ...
    [ "$ip" = "PENDING" ] && { log "$name $id has no ip/port yet"; live=$((live+1)); continue; }
    ...
    if { [ "$GPU" = "0" ] || [ "$GPU" = "NA" ]; } && [ -n "$LA" ] && [ "$LA" -ge $STALL_S ] 2>/dev/null; then
      kill_pod "$name" "$id" "$ip" "$port" "STALL KILL (GPU ${GPU}, python present, no log write for ${LA}s)"; [ -f "$WD/$name.gone" ] && live=$((live-1)); continue; fi
