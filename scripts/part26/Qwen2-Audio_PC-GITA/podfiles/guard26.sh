#!/bin/bash
# PART26 CPU pod guard, independent of the Mac: podStop via the pod's own key when ALL_DONE is older than 45 min
...
mkdir -p /workspace/logs; L=/workspace/logs/guard26.log; t0=$(date +%s)
echo "$(date -u +%H:%M:%S) guard start pod $RUNPOD_POD_ID key=$([ -n "$RUNPOD_API_KEY" ] && echo yes || echo no)" >> $L
...
while true; do
  now=$(date +%s)
  if [ -f /workspace/ALL_DONE ]; then a=$(( now - $(stat -c %Y /workspace/ALL_DONE) )); [ $a -gt 2700 ] && stop "ALL_DONE for ${a}s"; fi
