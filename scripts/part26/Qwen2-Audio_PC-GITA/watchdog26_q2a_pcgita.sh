#!/bin/bash
# PART26 watchdog for the Qwen2-Audio PC-GITA CPU pod (copy of part24/watchdog24.sh; changes: paths, pods file, sha check over every out/ file, a CPU pod with no GPU (GPU=NA) and no python counts as idle). Runs on the Mac, independent of any other process. Every 3 min, for each pod in pods24.txt
...
R="scores/part26/Qwen2-Audio_PC-GITA"
PODS="$R/pods26_q2a_pcgita.txt"; PULL="$R/pull"; WD="$R/wd"; mkdir -p "$PULL" "$WD"
KEY=$(tr -d '\n\r ' < ~/.runpod_key)
