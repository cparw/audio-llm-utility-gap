#!/bin/bash
# PART26 Kimi-Audio PC-GITA GPU pod driver. 1 Kimi env and model (part20 POD4d setup_kimi.sh, unchanged);
# 2 wait for the Mac upload, check sha256 of the tar and of every wav; 3 smoke run on 3 clips; 4 encoder states for all
# 1100 clips (kimi_enc_extract.py, new PART26 script); 5 rerun of the first 10 clips in a fresh process (determinism); ALL_DONE.
...
[ -f KIMI_PIP_DONE ] && [ -f KIMI_DL_DONE ] && log "setup already done, skipped (restart after the v1 untar failure)" || bash setup_kimi.sh
...
tar --no-same-owner -xf in/pcgita1100.tar -C data || fail "untar failed"   # v2: --no-same-owner, /workspace refuses chown (v1 failed here)
( cd data && sha256sum -c ../in/wav_sha256.txt > ../logs/wav_check.log 2>&1 ) || fail "wav sha256 check failed"
log "DATA OK $(grep -c ': OK' logs/wav_check.log) wavs"
/opt/kimienv/bin/python kimi_enc_extract.py in/mf_pcgita_pod.csv out/smoke_kimi_pcgita pd 3 > logs/smoke.log 2>&1 || fail "smoke failed"
log "SMOKE OK: $(grep RESULT logs/smoke.log)"
touch SMOKE_OK
/opt/kimienv/bin/python kimi_enc_extract.py in/mf_pcgita_pod.csv out/kimi_pcgita pd > logs/extract.log 2>&1 || fail "extract failed"
log "EXTRACT DONE: $(grep RESULT logs/extract.log)"
/opt/kimienv/bin/python kimi_enc_extract.py in/mf_pcgita_pod.csv out/rerun10_kimi_pcgita pd 10 > logs/rerun10.log 2>&1 || log "rerun10 failed"
