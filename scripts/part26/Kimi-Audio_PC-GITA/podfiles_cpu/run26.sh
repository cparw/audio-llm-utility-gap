#!/bin/bash
# PART26 Kimi-Audio PC-GITA CPU pod driver (copy of the Qwen2-Audio PC-GITA run26.sh, file names changed). 1 pinned fit libraries (sklearn 1.9.1, numpy 2.1.2, scipy 1.18.1);
...
python3 p25a_nested5.py in/kimi_pcgita_encstates.npz pcgita folds out/p26_kimi_pcgita > logs/probe.log 2>&1 || fail "probe failed"
log "PROBE DONE: $(grep 'NESTED5 DONE' logs/probe.log)"
python3 p25a_podverify.py in/kimi_pcgita_encstates.npz pcgita folds out/p26_kimi_pcgita > logs/podverify.log 2>&1 || log "podverify crashed"
