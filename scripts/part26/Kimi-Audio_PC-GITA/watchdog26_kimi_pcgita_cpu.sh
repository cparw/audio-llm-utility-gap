#!/bin/bash
# PART26 watchdog for the Kimi-Audio PC-GITA CPU probe pod (copy of part26/Qwen2-Audio_PC-GITA/watchdog26_q2a_pcgita.sh, itself a copy of part24/watchdog24.sh; changes: paths, pods file, sha check over every out/ file, a CPU pod with no GPU (GPU=NA) and no python counts as idle). Runs on the Mac, independent of any other process. Every 3 min, for each pod in pods24.txt
...
PODS="$R/pods26_kimi_pcgita_cpu.txt"; PULL="$R/pull"; WD="$R/wd"; mkdir -p "$PULL" "$WD"
KEY=$(tr -d '\n\r ' < ~/.runpod_key)
