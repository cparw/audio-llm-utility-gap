#!/bin/bash
# PART26 Kimi-Audio PC-GITA watchdog (copy of part24/watchdog24.sh; only the paths, the pulled file list, the result-hash glob and IDLE_S 900 to 1800 s changed; the Kimi pod waits idle for the 0.5 GB audio upload). Runs on the Mac, independent of any other process. Every 3 min, for each pod in pods24.txt
...
R="scores/part26/Kimi-Audio_PC-GITA"
PODS="$R/pods26_kimi_pcgita.txt"; PULL="$R/pull"; WD="$R/wd"; mkdir -p "$PULL" "$WD"
KEY=$(tr -d '\n\r ' < ~/.runpod_key)
...
ONCE=${1:-}
IDLE_S=1800; STALL_S=1200; UNREACH_S=1800; MAXAGE_S=14400; BAL_MIN=4
...
pull_pod(){ # name ip port -> 0 when the tar holds exactly the pod's file list and the result csv hashes match
  ...
  cnt=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace && { find out logs -type f 2>/dev/null; ls -1 *.sh *.py in_sha256.txt in/mf_pcgita_pod.csv in/wav_sha256.txt 2>/dev/null;
      for m in KIMI_PIP_DONE KIMI_DL_DONE PIP_DONE SMOKE_OK ALL_DONE FAILED; do [ -f $m ] && echo $m; done;
       } > /root/pull.list;
  ...
  ssh -n $SSHO -p $port root@$ip 'cd /workspace && sha256sum out/*.npz out/*.csv out/*.json 2>/dev/null' > "$WD/$n.remote.sha" 2>/dev/null
...
while true; do
  ...
  while read -r name id ip port cost <&3; do
    ...
    out=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace 2>/dev/null; a=0; [ -f ALL_DONE ] && a=1;
      ...
      mk=$(ls KIMI_PIP_DONE KIMI_DL_DONE PIP_DONE SMOKE_OK ALL_DONE 2>/dev/null | tr "\n" ",");
  ...
  log "live pods: $live"
  if [ $live -le 0 ]; then log "no live pods in pods26_kimi_pcgita.txt, watchdog exit"; exit 0; fi
