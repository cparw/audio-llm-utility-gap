#!/bin/bash
# PART 26 watchdog for the Kimi-Audio E-DAIC cell (copied from part24/watchdog24.sh; pull list, result hashes, markers and the CPU idle rule adapted). Runs on the Mac, independent of any other process. Every 3 min, for each pod in pods24.txt
...
PODS="$R/pods26_kimi_edaic.txt"; PULL="$R/pull"; WD="$R/wd"; mkdir -p "$PULL" "$WD"
KEY=$(tr -d '\n\r ' < ~/.runpod_key)
...
while true; do
  ...
  log "live pods: $live"
  if [ $live -le 0 ]; then log "no live pods in pods26_kimi_edaic.txt, watchdog exit"; exit 0; fi
