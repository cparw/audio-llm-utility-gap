#!/bin/bash
# PART 26 watchdog for the Kimi-Audio E-DAIC cell, try 2 (CPU probe pod only) (copied from part24/watchdog24.sh; pull list, result hashes, markers and the CPU idle rule adapted). Runs on the Mac, independent of any other process. Every 3 min, for each pod in pods24.txt
...
R="scores/part26/Kimi-Audio_E-DAIC"
PODS="$R/pods26_kimi_edaic_try2.txt"; PULL="$R/pull"; WD="$R/wd"; mkdir -p "$PULL" "$WD"
KEY=$(tr -d '\n\r ' < ~/.runpod_key)
...
pull_pod(){ # name ip port -> 0 when the tar holds exactly the pod's file list and the result csv hashes match
  ...
  cnt=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace && { find out logs -type f 2>/dev/null; ls -1 *.sh *.py out_sha256.txt 2>/dev/null;
      for m in KIMI_SETUP_DONE KIMI_IMPORT_OK PIP_DONE DATA_OK ALL_DONE FAILED; do [ -f $m ] && echo $m; done;
      for f in data/tarball.sha256 data/UPLOAD_OK in/UPLOAD_OK; do [ -f $f ] && echo $f; done; } > /root/pull.list;
...
while true; do
  ...
  while read -r name id ip port cost <&3; do
    ...
    out=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace 2>/dev/null; a=0; [ -f ALL_DONE ] && a=1;
      ...
      mk=$(ls KIMI_SETUP_DONE KIMI_IMPORT_OK PIP_DONE ALL_DONE 2>/dev/null | tr "\n" ",");
  ...
  log "live pods: $live"
  if [ $live -le 0 ]; then log "no live pods in pods26_kimi_edaic_try2.txt, watchdog exit"; exit 0; fi
