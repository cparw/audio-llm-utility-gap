#!/bin/bash
# PART 26 pod-side guard (copy of part26 guard26.sh; marker KIMI_SETUP_DONE instead of HF_DONE). Independent of the Mac:
...
while true; do
  ...
  if { [ "$g" = "0" ] || [ -z "$g" ]; } && [ "$py" = "0" ] && [ -f /workspace/KIMI_SETUP_DONE ]; then idle=$((idle+60)); else idle=0; fi
