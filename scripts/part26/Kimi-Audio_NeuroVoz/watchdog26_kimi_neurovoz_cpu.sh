#!/bin/bash
# PART26 Kimi-Audio NeuroVoz CPU pod watchdog (copy of the Qwen2-Audio NeuroVoz CPU watchdog, itself copied from part24/watchdog24.sh; WD dir wd_cpu; CPU pod: GPU "NA" counts as idle, as in part25 C). Runs on the Mac, independent of any other process. Every 3 min, for each pod in pods24.txt
...
# Exits when pods.txt has no live pods.   usage: bash watchdog26_kimi_neurovoz_cpu.sh [once]
R="scores/part26/Kimi-Audio_NeuroVoz"
PODS="$R/pods26_cpu.txt"; PULL="$R/pull"; WD="$R/wd_cpu"; mkdir -p "$PULL" "$WD"
KEY=$(tr -d '\n\r ' < ~/.runpod_key)
...
pull_pod(){ # name ip port -> 0 when the tar holds exactly the pod's file list and the result csv hashes match
  ...
  cnt=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace && { find out logs folds -type f 2>/dev/null; ls -1 *.sh *.py in/upload.sha256 2>/dev/null;
...
while true; do
  ...
  while read -r name id ip port cost <&3; do
    ...
    out=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace 2>/dev/null; a=0; [ -f ALL_DONE ] && a=1;
      ...
      mk=$(ls PIP_DONE ALL_DONE 2>/dev/null | tr "\n" ",");
