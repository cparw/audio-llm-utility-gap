#!/bin/bash
# PART26 CPU pod-side guard (independent of the Mac): podStop through the pod's own key when ALL_DONE is older than
# 45 min (the Mac watchdog should have pulled and deleted by then), or no python for 40 min after PIP_DONE, or 3 h uptime.
