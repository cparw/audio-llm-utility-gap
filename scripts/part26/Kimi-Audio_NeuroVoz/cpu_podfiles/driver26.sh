#!/bin/bash
# PART26 Kimi-Audio NeuroVoz CPU pod driver (copy of the Qwen2-Audio NeuroVoz driver26.sh, file names swapped). usage: nohup bash driver26.sh &
...
python3 p25a_nested5.py in/kimi_neurovoz_encstates.npz neurovoz folds out/p26_kimi_neurovoz > logs/probe_neurovoz.log 2>&1 || fail "probe failed"
log "PROBE DONE: $(grep 'NESTED5 DONE' logs/probe_neurovoz.log)"
python3 p25a_podverify.py in/kimi_neurovoz_encstates.npz neurovoz folds out/p26_kimi_neurovoz > logs/podverify_neurovoz.log 2>&1 || log "podverify crashed"
