#!/bin/bash
# PART26 Kimi-Audio NeuroVoz watchdog (copied from part24/watchdog24.sh; pull list, markers and result hashes set for this cell; log age also counts
# writes under /workspace/hf so a model download is not taken for a stall). Runs on the Mac, independent of any other process. Every 3 min, for each pod in pods24.txt
...
# Exits when pods26.txt has no live pods.   usage: bash watchdog26_kimi_neurovoz.sh [once]
...
pull_pod(){ # name ip port -> 0 when the tar holds exactly the pod's file list and the result csv hashes match
  ...
  cnt=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace && { find out logs -type f 2>/dev/null; ls -1 *.sh *.py in/*.csv in/*.txt 2>/dev/null;
      for m in KIMI_PIP_DONE KIMI_DL_DONE SMOKE_OK ALL_DONE FAILED PIP_DONE; do [ -f $m ] && echo $m; done;
...
while true; do
  ...
  while read -r name id ip port cost <&3; do
    ...
    out=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace 2>/dev/null; a=0; [ -f ALL_DONE ] && a=1;
      ...
      lm=$(find /workspace/logs /workspace/hf -type f -printf "%T@\n" 2>/dev/null | sort -n | tail -1 | cut -d. -f1); nw=$(date +%s);
      f=-; [ -f FAILED ] && f=$(cat FAILED);
      mk=$(ls KIMI_PIP_DONE KIMI_DL_DONE in/UPLOAD_OK SMOKE_OK PIP_DONE ALL_DONE 2>/dev/null | tr "\n" ",");
