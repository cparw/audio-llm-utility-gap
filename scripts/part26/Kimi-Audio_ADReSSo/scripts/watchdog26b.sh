#!/bin/bash
# PART26 Kimi-Audio ADReSSo watchdog, second try (pods26b.txt) (copied from part24/watchdog24.sh; changes: paths, pods26.txt, CPU pods count GPU=NA as 0%, result sha check on out/*, max age 2 h, UPLOAD_SHA256.txt and mf_*.csv pulled).
...
R="scores/part26/Kimi-Audio_ADReSSo"
PODS="$R/pods26b.txt"; PULL="$R/pull"; WD="$R/wd"; mkdir -p "$PULL" "$WD"
KEY=$(tr -d '\n\r ' < ~/.runpod_key)
...
pull_pod(){ # name ip port -> 0 when the tar holds exactly the pod's file list and the result csv hashes match
  ...
  cnt=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace && { find out logs p23 smoke_out -type f 2>/dev/null; ls -1 *.sh *.py UPLOAD_SHA256.txt mf_*.csv 2>/dev/null;
...
while true; do
  ...
  while read -r name id ip port cost <&3; do
    ...
    ALL=$(echo "$out" | sed -n 's/.*ALL=\([01]\).*/\1/p'); GPU=$(echo "$out" | sed -n 's/.*GPU=\([0-9NA]*\).*/\1/p'); [ "$GPU" = "NA" ] && GPU=0
  ...
  log "live pods: $live"
  if [ $live -le 0 ]; then log "no live pods in pods24.txt, watchdog exit"; exit 0; fi
