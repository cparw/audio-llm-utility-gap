#!/bin/bash
# PART26 watchdog, cell Qwen2-Audio E-DAIC (copied from part24/watchdog24.sh; changes: paths, CPU pod GPU=0, pull list,
# result sha check on out/*, max age 2 h). Runs on the Mac, independent of any other process. Every 3 min, for each pod in pods26.txt
...
pull_pod(){ # name ip port -> 0 when the tar holds exactly the pod's file list and the result csv hashes match
  ...
  cnt=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace && { find out logs folds -type f 2>/dev/null; ls -1 *.sh *.py out_sha256.txt in/files.sha256 2>/dev/null;
      for m in PIP_DONE ALL_DONE FAILED; do [ -f $m ] && echo $m; done;
...
while true; do
  ...
  while read -r name id ip port cost <&3; do
    ...
    out=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace 2>/dev/null; a=0; [ -f ALL_DONE ] && a=1;
      ...
      echo "ALL=$a GPU=${g:-0} PY=$py LOGAGE=$((nw-${lm:-$nw})) FAILED=$f MK=$mk | $(tail -1 logs/DRIVER.log 2>/dev/null | cut -c1-150)"' 2>/dev/null | tr '\n' ' ')
    if [ -z "$out" ]; then
