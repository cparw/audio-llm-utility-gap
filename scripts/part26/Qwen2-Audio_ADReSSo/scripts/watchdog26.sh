#!/bin/bash
# PART26 watchdog (copied from part24/watchdog24.sh; changes: paths, CPU pods count GPU=NA as 0%, result sha check on out/*, max age 2 h, UPLOAD_SHA256.txt pulled).
...
R="scores/part26/Qwen2-Audio_ADReSSo"
...
pull_pod(){ # name ip port -> 0 when the tar holds exactly the pod's file list and the result csv hashes match
  ...
  cnt=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace && { find out logs p23 smoke_out -type f 2>/dev/null; ls -1 *.sh *.py UPLOAD_SHA256.txt 2>/dev/null;
