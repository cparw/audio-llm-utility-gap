#!/bin/bash
# PART 26 pod driver, Qwen2-Audio ADReSSo encoder nested probe (five saved repeat folds + single split).
# Installs the pinned versions, checks the uploaded files against UPLOAD_SHA256.txt, runs p25a_nested5.py
# (byte copy of the PART 25 A script) with one BLAS thread per fit, then touches ALL_DONE. The Mac watchdog pulls
# out/, logs/ and the driver files, then deletes the pod.
...
python3 -u p25a_nested5.py in/q2a_adresso_enc_states.npz adresso folds out/p26_q2a_adresso > logs/nested5.log 2>&1 \
  && log "done $(tail -1 logs/nested5.log)" || { log "FAILED nested5"; echo nested5 >> FAILED; }
