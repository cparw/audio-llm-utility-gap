"""PART 26 independent check of the Qwen2-Audio ADReSSo cell. Written separately from gap26.py and p25a_nested5.py.
...
3. Nested probe rerun on the Mac from the ORIGINAL states file (probe2/adresso_states.npz, key enc), with an own
...
Writes verify/verify26.json.   usage: /usr/local/bin/python3 verify26.py"""
import os
...
pod_rows = {r["clip"]: r for r in csv.DictReader(open(f"{POD}/out/p26_q2a_adresso_enc_nested5_oof.csv"))}
pc_rows = list(csv.DictReader(open(f"{C}/per_clip/Qwen2-Audio_ADReSSo_perclip.csv")))
...
side = json.load(open(f"{C}/per_clip/Qwen2-Audio_ADReSSo_perclip.sidecar.json"))
...
pod_json = json.load(open(f"{POD}/out/p26_q2a_adresso_enc_nested5.json"))
t0 = time.time(); mac = {}
for s in range(5):
...
# 4. bootstrap rebuilt
D = np.load(f"{C}/draws/Qwen2-Audio_ADReSSo_draws.npz", allow_pickle=True)
...
json.dump(out, open(f"{C}/verify/verify26.json", "w"), indent=1)
