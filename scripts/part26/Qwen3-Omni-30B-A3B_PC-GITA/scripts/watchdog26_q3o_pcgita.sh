#!/bin/bash
# PART26 watchdog (copied from part24/watchdog24.sh; changes: paths, pod list, CPU idle rule, pull list, result sha list,
# max age 3 h). Runs on the Mac, independent of any other process. Every 3 min, for each pod in pods26.txt
...
# Exits when pods24.txt has no live pods.   usage: bash watchdog24.sh [once]
R="scores/part26/Qwen3-Omni-30B-A3B_PC-GITA"
PODS="$R/launch/pods26.txt"; PULL="$R/pull"; WD="$R/wd"; mkdir -p "$PULL" "$WD"
KEY=$(tr -d '\n\r ' < ~/.runpod_key)
...
ONCE=${1:-}
IDLE_S=900; STALL_S=1200; UNREACH_S=1800; MAXAGE_S=10800; BAL_MIN=4
...
pull_pod(){ # name ip port -> 0 when the tar holds exactly the pod's file list and the result csv hashes match
  ...
  cnt=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace && { find out logs -type f 2>/dev/null; ls -1 *.sh *.py out_sha256.txt in/expected.sha256 2>/dev/null;
      for m in PIP_DONE in/UPLOAD_OK ALL_DONE FAILED; do [ -f $m ] && echo $m; done;
  ...
  ssh -n $SSHO -p $port root@$ip 'cd /workspace && sha256sum out/*_nested5_oof.csv out/*_nested5.json out/*_podverify.json 2>/dev/null' > "$WD/$n.remote.sha" 2>/dev/null
...
while true; do
  ...
  while read -r name id ip port cost <&3; do
    ...
    out=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace 2>/dev/null; a=0; [ -f ALL_DONE ] && a=1;
      ...
      mk=$(ls PIP_DONE in/UPLOAD_OK ALL_DONE 2>/dev/null | tr "\n" ",");
    ...
    if { [ "$GPU" = "0" ] || [ "$GPU" = "NA" ]; } && [ "$PY" = "0" ]; then
      ...
      log "$name idle (CPU pod, no python) for ${i}s"
    ...
    if { [ "$GPU" = "0" ] || [ "$GPU" = "NA" ]; } && [ -n "$LA" ] && [ "$LA" -ge $STALL_S ] 2>/dev/null; then
      kill_pod "$name" "$id" "$ip" "$port" "STALL KILL (python present, no log write for ${LA}s)"; [ -f "$WD/$name.gone" ] && live=$((live-1)); continue; fi
