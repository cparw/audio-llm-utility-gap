#!/bin/bash
# PART 26 CPU pod-side guard (independent of the Mac): podStop via the pod's own key when ALL_DONE is older than 45 min
# (the Mac watchdog should have pulled and deleted by then), or no python for 40 min after PIP_DONE, or after 3 h uptime.
eval $(cat /proc/1/environ | tr "\0" "\n" | grep -E "^RUNPOD_(POD_ID|API_KEY)=" | sed "s/^/export /")
mkdir -p /workspace/logs; L=/workspace/logs/guard26.log; idle=0; t0=$(date +%s)
echo "$(date -u +%H:%M:%S) guard start pod $RUNPOD_POD_ID key $([ -n "$RUNPOD_API_KEY" ] && echo present || echo MISSING)" >> $L
stop(){ echo "$(date -u +%H:%M:%S) GUARD STOP: $1" >> $L; echo "$(date -u +%H:%M:%S) GUARD_STOP $1" >> /workspace/logs/DRIVER.log; sync
