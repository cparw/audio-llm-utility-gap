#!/bin/bash
# PART26 CPU pod driver, Kimi-Audio MDVR-KCL encoder nested probe.
...
python3 p25a_nested5.py in/kimi_kcl_encstates.npz kcl folds out/p26_kimi_kcl > logs/probe_kcl.log 2>&1 || fail "probe failed: $(tail -2 logs/probe_kcl.log | tr '\n' ' ')"
log "PROBE DONE: $(grep 'NESTED5 DONE' logs/probe_kcl.log)"
python3 p25a_podverify.py in/kimi_kcl_encstates.npz kcl folds out/p26_kimi_kcl > logs/podverify_kcl.log 2>&1 || log "podverify crashed"
