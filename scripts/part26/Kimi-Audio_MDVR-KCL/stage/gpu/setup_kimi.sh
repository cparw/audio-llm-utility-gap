#!/bin/bash
# PART26 copy of release/edaic_conflict_new/kimi/setup_kimi.sh. Two edits: the model snapshot is pinned to the revision every
# earlier Kimi run used (9a82a84c37ad9eb1307fb6ed8d7b397862ef9e6b), and the Kimi-Audio git commit is logged.
...
echo "KIMI_SRC_COMMIT $(git -C /workspace/kimi_src rev-parse HEAD) glm4 $(git -C /workspace/kimi_src/kimia_infer/models/tokenizer/glm4 rev-parse HEAD)"
