#!/bin/bash
# PART26 watchdog for the Kimi-Audio MDVR-KCL cell (copy of part24/watchdog24.sh with the part26 edits: idle also when no GPU reading (CPU pod) and no python, result sha check on out/*, *.txt and *.sha256 pulled, 2 h max age; STALL at 40 min because the GPU setup downloads the model with no log writes).
