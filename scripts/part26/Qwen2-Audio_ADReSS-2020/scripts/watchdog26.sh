#!/bin/bash
# PART26 watchdog, cell Qwen2-Audio ADReSS-2020 (copied from part24/watchdog24.sh, adapted for a CPU pod as part25/C did:
# no GPU, so 'GPU NA' counts as GPU 0%). Runs on the Mac, independent of any other process. Every 3 min, for each pod in pods26.txt
...
R="scores/part26/Qwen2-Audio_ADReSS-2020"
...
pull_pod(){ # name ip port -> 0 when the tar holds exactly the pod's file list and the result csv hashes match
  ...
  cnt=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace && { find out logs -type f 2>/dev/null; ls -1 *.sh *.py out_sha256.txt in.sha256 folds/*.csv 2>/dev/null;
      for m in PIP_DONE UPLOAD_OK ALL_DONE FAILED; do [ -f $m ] && echo $m; done; } > /root/pull.list;
...
while true; do
  ...
  while read -r name id ip port cost <&3; do
    ...
    out=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace 2>/dev/null; a=0; [ -f ALL_DONE ] && a=1;
      ...
      mk=$(ls PIP_DONE UPLOAD_OK ALL_DONE 2>/dev/null | tr "\n" ",");
