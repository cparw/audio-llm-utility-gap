"""PART26 copy of PART25 A p25a_nested5.py (md5 615ced0380cca2636ed7cc30c71810e6). Only three edits, estimator unchanged:
 (1) TAGS come from env P26_TAGS (default the five seeds; no "single" split, since no single-split fold file exists for the
     Control15-style Pitt ids of the part16 POD3 Qwen3-Omni states);
 (2) the fold file stem comes from env P26_FOLD_STEM (default "pod"), so pitt_groupkfold5_pod_Control15ids_seedS.csv is read;
 (3) single_split_auc is written only when the single tag was run.
...
def fold_file(tag):
    ...
    cands = ([f"{DS}_groupkfold5_pod.csv"] if tag == "single" else
             [f"{DS}_groupkfold5_{stem}_{tag}.csv", f"{DS}_groupkfold5_{stem}_{tag}_UNVERIFIED.csv"])
    for c in cands:
...
res = {"dataset": DS, "stream": KEY, "n": int(len(y)), "n_pos": int(y.sum()), "n_speakers": int(len(np.unique(spk))),
       ...
       "single_split_auc": ck["single"]["auc"] if "single" in ck else None,
...
print(f"NESTED5 DONE {DS} mean5 {res['mean5']:.6f} per {[round(a, 4) for a in per]} single {res['single_split_auc']}", flush=True)
