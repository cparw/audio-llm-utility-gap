#!/bin/bash
# PART26 CPU pod-side guard (independent of the Mac): podStop with the pod's own key when ALL_DONE is older than
# 45 min (the Mac watchdog should have pulled and deleted by then), or no python for 40 min after PIP_DONE,
# or after 3 h uptime. CPU pod, so there is no GPU check.
