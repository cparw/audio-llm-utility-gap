#!/bin/bash
# PART26 watchdog for the Qwen3-Omni Pitt CPU pod (copied from part24/watchdog24.sh; edits: paths, pods26.txt,
# CPU pods count GPU=NA as idle, pull list and hash check for this job, max age 2 h, stall 40 min,
# PART 26 spend floor: pull and delete when the balance drops below $36.72 = $126.72 - $90).
# Original header follows.
# PART24 watchdog. Runs on the Mac, independent of any other process. Every 3 min, for each pod in pods24.txt
...
ONCE=${1:-}
IDLE_S=900; STALL_S=2400; UNREACH_S=1800; MAXAGE_S=7200; BAL_MIN=36.72
...
pull_pod(){ # name ip port -> 0 when the tar holds exactly the pod's file list and the result csv hashes match
  ...
  cnt=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace && { find out logs -type f 2>/dev/null; ls -1 *.sh *.py files.sha256 2>/dev/null;
