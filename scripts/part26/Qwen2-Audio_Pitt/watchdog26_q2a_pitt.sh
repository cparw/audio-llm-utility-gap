#!/bin/bash
# PART26 Qwen2-Audio Pitt watchdog (copied from part24/watchdog24.sh, CPU-pod edits as in part25/C/watchdog25C.sh). Runs on the Mac, independent of any other process. Every 3 min, for each pod in pods26.txt
...
# Exits when pods26.txt has no live pods.   usage: bash watchdog26_q2a_pitt.sh [once]
R="scores/part26/Qwen2-Audio_Pitt"
...
pull_pod(){ # name ip port -> 0 when the tar holds exactly the pod's file list and the result csv hashes match
  ...
  cnt=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace && { find out logs folds -type f 2>/dev/null; ls -1 *.sh *.py files.sha256 out_sha256.txt 2>/dev/null;
