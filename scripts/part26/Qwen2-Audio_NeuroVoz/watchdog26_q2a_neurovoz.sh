#!/bin/bash
# PART26 Qwen2-Audio NeuroVoz watchdog (copied from part24/watchdog24.sh; CPU pod: GPU "NA" counts as idle, as in part25 C). Runs on the Mac, independent of any other process. Every 3 min, for each pod in pods24.txt
...
# Exits when pods.txt has no live pods.   usage: bash watchdog26_q2a_neurovoz.sh [once]
