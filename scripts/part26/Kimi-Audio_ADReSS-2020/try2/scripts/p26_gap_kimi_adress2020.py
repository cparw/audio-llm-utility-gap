"""PART26: Kimi-Audio, ADReSS-2020 (try2; copy of the MDVR-KCL cell gap script, paths and dataset changed). Paired probe minus zero-shot answer gap on the paper's estimator (mean of five
...
32 Whisper-large-v3 encoder layers inside kimia_infer, mean over the token_len*4 frames Kimi keeps), checked by
p26_states_check_kimi_adress2020.py before the probe ran.
...
Zero-shot: release/overnight2/kimi_new/kimi_adress2020_zeroshot_scores.csv (read only), the file behind the master 0.7053.
Writes per_clip/p26_kimi_adress2020_perclip.csv, per_clip/p26_kimi_adress2020_perclip.sidecar.json, draws/p26_kimi_adress2020_draws.npz."""
import os, sys, csv, json, hashlib, datetime, platform, numpy as np
import sklearn
from sklearn.metrics import roc_auc_score

R = "scores/part26/Kimi-Audio_ADReSS-2020"
POD = f"{R}/try2/pull/p26-kimi-adress2020-c2"
GPOD = f"{R}/try2/pull/p26-kimi-adress2020-g3"
OOF = f"{POD}/out/p26_kimi_adress2020_enc_nested5_oof.csv"; JS = f"{POD}/out/p26_kimi_adress2020_enc_nested5.json"
PV = f"{POD}/out/p26_kimi_adress2020_podverify.json"
ZS = "<local data dir>/release/overnight2/kimi_new/kimi_adress2020_zeroshot_scores.csv"
STATES = f"{GPOD}/out/p26_kimi_adress2020_states.npz"
SCHK = f"{R}/verify/p26_states_check_kimi_adress2020.json"
NB = 2000
...
DR = f"{R}/draws/p26_kimi_adress2020_draws.npz"
...
PC = f"{R}/per_clip/p26_kimi_adress2020_perclip.csv"
...
rec = {"task": "PART26 probe grid: Kimi-Audio encoder nested probe (layer chosen inside the training folds), five repeats on the saved fold files, paired probe minus zero-shot answer gap",
       ...
       "model": "Kimi-Audio-7B-Instruct", "dataset": "ADReSS-2020", "condition": "AD",
       ...
       "states": {"script": "try2/stage/gpu/kimi_enc_probe.py (byte equal to the MDVR-KCL cell copy, md5 13a643f175b363f7ce6d37b5bc27eebc: podD2_final/kimi_probe.py plus Whisper encoder hooks)",
                  "setup": "try2/stage/gpu/setup_kimi.sh (MDVR-KCL cell copy of edaic_conflict_new/kimi/setup_kimi.sh, snapshot pinned to 9a82a84c37ad9eb1307fb6ed8d7b397862ef9e6b)",
                  "window": "first 30 s (x[:16000*30]) of the same 156 clips as the zero-shot answer; 142 are 30.0 s, 14 are shorter (4.33 to 28.74 s)",
                  "encoder": "32 Whisper-large-v3 layers, 1280 dims, bf16; each layer output cut to the token_len*4 frames Kimi keeps (1500 for a 30 s clip) and mean-pooled (h.mean(dim=0).float()), as the Qwen2.5-Omni encoder states",
...
rec["attempt"] = "try2: try 1 (01:00 to 01:43 UTC) never extracted; its Mac upload never arrived and the watchdog idle-killed the GPU pod after setup"
json.dump(rec, open(f"{R}/per_clip/p26_kimi_adress2020_perclip.sidecar.json", "w"), indent=1)
print(f"Kimi-Audio ADReSS-2020 encoder probe mean5 {m5:.4f} per {[round(a, 4) for a in per]}  answer {zauc:.4f}  "
