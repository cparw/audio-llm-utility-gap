"""PART26 independent Mac check of the Kimi-Audio ADReSS-2020 cell (try2). Copy of the MDVR-KCL cell verify script with paths,
dataset and the master zero-shot value (0.7053) changed. Written separately from p26_gap_kimi_adress2020.py (no shared code).
 ...
 5 zero-shot AUC recomputed exactly from the saved file; equals the master 0.7053 at 4 dp; clip set, labels, speakers equal;
...
Writes verify/p26_verify_kimi_adress2020.json."""
...
POD = f"{R}/try2/pull/p26-kimi-adress2020-c2"; GPOD = f"{R}/try2/pull/p26-kimi-adress2020-g3"
...
# 2
J = json.load(open(f"{POD}/out/p26_kimi_adress2020_enc_nested5.json"))
gs = sha(f"{GPOD}/out/p26_kimi_adress2020_states.npz")
ck("probed_states_equal_gpu_states", J["states"]["sha256"] == gs, pod_json_sha=J["states"]["sha256"], gpu_sha=gs)
s = np.load(f"{GPOD}/out/p26_kimi_adress2020_states.npz", allow_pickle=True)
names = [str(x) for x in s["name"]]; spk = [str(x) for x in s["spk"]]; lab = [int(x) for x in s["label"]]
# 3
rows = list(csv.DictReader(open(f"{POD}/out/p26_kimi_adress2020_enc_nested5_oof.csv")))
...
fok = {}
for t, fn in [(f"seed{i}", f"adress2020_groupkfold5_pod_seed{i}_UNVERIFIED.csv") for i in range(5)] + [("single", "adress2020_groupkfold5_pod.csv")]:
...
ck("zeroshot_auc_exact_equals_0.7053_4dp", round(float(zauc), 4) == 0.7053, exact=str(zauc), value=float(zauc))
# 6
S = json.load(open(f"{R}/per_clip/p26_kimi_adress2020_perclip.sidecar.json"))
pc = list(csv.DictReader(open(f"{R}/per_clip/p26_kimi_adress2020_perclip.csv")))
...
dr = np.load(f"{R}/draws/p26_kimi_adress2020_draws.npz", allow_pickle=True)
...
# 8
PV = json.load(open(f"{POD}/out/p26_kimi_adress2020_podverify.json"))
...
json.dump(res, open(f"{R}/verify/p26_verify_kimi_adress2020.json", "w"), indent=1, default=str)
