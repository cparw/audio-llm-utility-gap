#!/bin/bash
# PART26 watchdog for the Kimi-Audio ADReSS-2020 cell, try2 (copy of part24/watchdog24.sh with the part26 edits: idle also when no GPU reading (CPU pod) and no python, result sha check on out/*, *.txt and *.sha256 pulled, 2 h max age; STALL at 40 min because the GPU setup downloads the model with no log writes).
...
R="scores/part26/Kimi-Audio_ADReSS-2020/try2"
...
ONCE=${1:-}
IDLE_S=900; STALL_S=2400; UNREACH_S=1800; MAXAGE_S=7200; BAL_MIN=4
...
pull_pod(){ # name ip port -> 0 when the tar holds exactly the pod's file list and the result csv hashes match
  ...
  cnt=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace && { find out logs p23 smoke_out -type f 2>/dev/null; ls -1 *.sh *.py *.txt *.sha256 2>/dev/null;
