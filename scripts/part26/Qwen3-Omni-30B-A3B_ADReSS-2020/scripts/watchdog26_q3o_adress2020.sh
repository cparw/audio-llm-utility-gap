#!/bin/bash
# PART26 watchdog for the Qwen3-Omni ADReSS-2020 CPU probe pod (copied from part24/watchdog24.sh; changes: paths,
# pods26.txt, 60 s loop, 2 h max age, CPU pods count as GPU 0% (no nvidia-smi -> NA), pull list adds folds/in_sha256,
# result sha check on out/*.csv and out/*.json). Runs on the Mac, independent of any other process. For each pod in pods26.txt
...
ONCE=${1:-}
IDLE_S=900; STALL_S=1200; UNREACH_S=1800; MAXAGE_S=7200; BAL_MIN=4
...
pull_pod(){ # name ip port -> 0 when the tar holds exactly the pod's file list and the result csv hashes match
  ...
  cnt=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace && { find out logs folds -type f 2>/dev/null; ls -1 *.sh *.py in_sha256.txt out_sha256.txt 2>/dev/null;
      for m in PIP_DONE UPLOAD_OK ALL_DONE FAILED; do [ -f $m ] && echo $m; done;
  ...
  return 0; }
gpu_idle(){ # GPU 0%, or no numeric GPU reading at all (CPU pod) -> idle
  [[ "$1" =~ ^[0-9]+$ ]] || return 0; [ "$1" = "0" ]; }
...
while true; do
  ...
  sl=$(( 60 - ($(date +%s) - now) )); [ $sl -lt 20 ] && sl=20; sleep $sl
done
