#!/bin/bash
# PART26 CPU pod-side guard (independent of the Mac): podStop with the pod's own key when ALL_DONE is older than 30 min
# (the Mac watchdog should have pulled and deleted by then), when no python has run for 40 min after PIP_DONE without
# ALL_DONE, or after 3 h uptime.
...
while true; do
  ...
  if [ "$py" = "0" ] && [ -f /workspace/PIP_DONE ] && [ ! -f /workspace/ALL_DONE ]; then idle=$((idle+60)); else idle=0; fi
  [ $idle -ge 2400 ] && stop "idle ${idle}s (no python, no ALL_DONE)"
