#!/bin/bash
# PART26 watchdog, cell Kimi-Audio Pitt (copied from part24/watchdog24.sh via the Qwen2-Audio ADReSS-2020 cell copy, which
# treats 'GPU NA' on a CPU pod as GPU 0%; this cell runs one GPU extraction pod, then one CPU probe pod). Runs on the Mac, independent of any other process. Every 3 min, for each pod in pods26.txt
...
# Exits when pods26.txt has no live pods.   usage: bash watchdog26.sh [once]
R="scores/part26/Kimi-Audio_Pitt"
PODS="$R/pods26.txt"; PULL="$R/pull"; WD="$R/wd"; mkdir -p "$PULL" "$WD"
KEY=$(tr -d '\n\r ' < ~/.runpod_key)
...
ONCE=${1:-}
IDLE_S=900; STALL_S=1800; UNREACH_S=1800; MAXAGE_S=14400; BAL_MIN=4
...
pull_pod(){ # name ip port -> 0 when the tar holds exactly the pod's file list and the result csv hashes match
  ...
  cnt=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace && { find out logs -type f 2>/dev/null; ls -1 *.sh *.py out_sha256.txt in.sha256 folds/*.csv in/*.csv in/*.txt in/*.sha256 2>/dev/null;
      for m in PIP_DONE UPLOAD_OK KIMI_PIP_DONE KIMI_DL_DONE in/UPLOAD_OK SMOKE_OK ALL_DONE FAILED; do [ -f $m ] && echo $m; done; } > /root/pull.list;
  ...
  ssh -n $SSHO -p $port root@$ip 'cd /workspace && sha256sum out/* 2>/dev/null' > "$WD/$n.remote.sha" 2>/dev/null
...
while true; do
  ...
  while read -r name id ip port cost <&3; do
    ...
    out=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace 2>/dev/null; a=0; [ -f ALL_DONE ] && a=1;
      ...
      mk=$(ls PIP_DONE UPLOAD_OK KIMI_PIP_DONE KIMI_DL_DONE in/UPLOAD_OK SMOKE_OK ALL_DONE 2>/dev/null | tr "\n" ",");
  ...
  log "live pods: $live"
  if [ $live -le 0 ]; then log "no live pods in pods26.txt, watchdog exit"; exit 0; fi
