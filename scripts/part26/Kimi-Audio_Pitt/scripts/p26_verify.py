"""PART26 cell Kimi-Audio / Pitt: independent check on the Mac, written separately from p26_gap.py and from the
pod scripts (no shared code). Checks:
 ...
 E. the pod input equals the encoder states pulled from this cell's GPU pod (enc, label, spk, name, p_yes) and its sha256
    equals the one the pod read; the pulled states match the GPU pod's own sha256 list;
...
res = {"checks": {}}
pod = list(csv.DictReader(open(f"{POD}/p26_kimi_pitt_enc_nested5_oof.csv")))
cell = list(csv.DictReader(open(f"{C}/{TAG}_perclip.csv")))
zsr = {r["clip"]: r for r in csv.DictReader(open(ZS))}
J = json.load(open(f"{POD}/p26_kimi_pitt_enc_nested5.json"))
...
b = {}
for s in range(5):
    fmap = {r["clip_id"]: (int(r["fold"]), r["speaker_id"]) for r in csv.DictReader(open(f"{FD}/pitt_groupkfold5_pod_seed{s}_UNVERIFIED.csv"))}
    ...
    b[f"seed{s}"] = {"pod_equals_file": bool(np.array_equal(ff, pc)), "own_fill_equals_file": bool(np.array_equal(own, ff)),
                     "speakers_equal_file": all(fmap[c][1] == x for c, x in zip(clips, spk)), "fold_file_sha256": h(f"{FD}/pitt_groupkfold5_pod_seed{s}_UNVERIFIED.csv")}
...
gl = dict((l.split()[1], l.split()[0]) for l in open(f"{C}/pull/p26-kimi-pitt-gpu/logs/out_sha256.txt") if l.strip())
res["checks"]["E_inputs"]["gpu_states_sha256_equals_pod_list"] = h(SRC) == gl.get("kimi_pitt_encstates.npz")
# F (information only)
...
if __name__ == "__main__":
    ...
    eE = res["checks"]["E_inputs"]; cE = eE["stage_arrays_equal_source"] and eE["sha_equal"] and eE["stage_names_equal_pod_order"] and eE["gpu_states_sha256_equals_pod_list"]
