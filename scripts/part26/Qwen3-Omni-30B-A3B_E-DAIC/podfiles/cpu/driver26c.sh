#!/bin/bash
# PART 26 Qwen3-Omni E-DAIC CPU pod driver: pinned fit libraries (sklearn 1.9.1, numpy 2.1.2, scipy 1.18.1), then the
# PART 25 A nested five-repeat encoder probe (p25a_nested5.py, unchanged) on the re-extracted states with the saved
# release fold files, one BLAS thread per fit, then the independent pod-side check (p25a_podverify.py). ALL_DONE.
...
log "DRIVER START q3o edaic probe"
...
python3 p25a_nested5.py in/p26_q3o_edaic_encstates.npz edaic folds out/p26_q3o_edaic > logs/probe_edaic.log 2>&1 || fail "probe failed: $(tail -2 logs/probe_edaic.log | tr '\n' ' ')"
log "PROBE DONE: $(grep 'NESTED5 DONE' logs/probe_edaic.log)"
python3 p25a_podverify.py in/p26_q3o_edaic_encstates.npz edaic folds out/p26_q3o_edaic > logs/podverify_edaic.log 2>&1 || log "podverify crashed: $(tail -2 logs/podverify_edaic.log | tr '\n' ' ')"
