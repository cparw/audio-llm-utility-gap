#!/bin/bash
# PART26 watchdog (copy of part24/watchdog24.sh; changes: paths, pods26.txt, CPU pods count as idle when GPU=NA and no python,
# result sha check covers every out/ file, max age 90 min). Runs on the Mac, independent of any other process. Every 3 min, for each pod in pods24.txt
...
ONCE=${1:-}
IDLE_S=900; STALL_S=1200; UNREACH_S=1800; MAXAGE_S=5400; BAL_MIN=4
