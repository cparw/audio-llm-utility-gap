#!/bin/bash
# PART26 watchdog for the Qwen3-Omni-30B-A3B MDVR-KCL cell, copied from part24/watchdog24.sh. Changes: paths, pod list
# pods26.txt, the pull list (out/, logs/, in/files.sha256, driver files), the sha256 check covers every out/ file,
# a CPU pod (no nvidia-smi, GPU=NA) counts as GPU 0% for the idle and stall rules, max age 3 h.
# Runs on the Mac, independent of any other process. Every 3 min, for each pod in pods26.txt
...
pull_pod(){ # name ip port -> 0 when the tar holds exactly the pod's file list and the result csv hashes match
  ...
  cnt=$(ssh -n $SSHO -p $port root@$ip 'cd /workspace && { find out logs -type f 2>/dev/null; ls -1 *.sh *.py out_sha256.txt in/files.sha256 2>/dev/null;
