"""PATCHED COPY: adds PROMPT_OVERRIDE and WINDOW_S env vars (WINDOW_S=0 means the full clip).
This is the exact variant used for the Part 1c match run against the cluster p_dep scores.
...
MDTYPE = torch.float16 if "Qwen2-Audio" in MID else torch.bfloat16
t0 = time.time()
IS_Q3 = "Qwen3-Omni" in MID
IS_OMNI = "Omni" in MID
if IS_Q3:
    ...
    net = net.to(dev).eval()
    tower = net.audio_tower; enc_layers = tower.layers
    projector = tower.proj2            # audio_tower connector is proj1 -> act -> proj2; proj2 is its output
elif IS_OMNI:
...
for i, r in enumerate(rows):
    ...
    for _k, _v in list(inp.items()):                       # audio features arrive float32; match the model dtype
...
json.dump({"checkpoint": MID, "clip_list": os.path.abspath(MANIFEST), "condition": COND, "prompt": P,
           ...
           "window_seconds": WIN, "dtype": "bfloat16" if (IS_OMNI or IS_Q3) else "float16", "device": dev,
